# Security Policy for `pathlib_gui`
...
1. **GitHub private vulnerability reporting** —
   [submit a confidential report](https://github.com/matthewdeanmartin/pathlib_gui/security/advisories/new)
...
For maximum isolation, run `pathlib_gui` inside a container so it only has access to what you
explicitly mount.
...
RUN pip install --no-cache-dir pathlib_gui

ENTRYPOINT ["pathlib_gui"]
CMD ["--help"]
```
...
```bash
docker build -t pathlib_gui .

# Mount the current directory into /data and pass it as an argument
docker run --rm -v "$(pwd):/data" pathlib_gui /data
```
